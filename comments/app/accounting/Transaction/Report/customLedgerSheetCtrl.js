(function () {
    ...
    angular
        ...
        .controller("customLedgerSheetCtrl", ["accCOAResource", "$filter", "Util", "ledgerSheetResource", "param", "appAuth", customLedgerSheetCtrl]);
    function customLedgerSheetCtrl(accCOAResource, $filter, Util, ledgerSheetResource, param, appAuth) {
        var vm = this;
        vm.title = param.LedgerSheetName;
        vm.ledgerSheets = [];
        vm.ledgerList = [];
        vm.ledgerSheetts = [];
        vm.totalCR = 0;
       ...
       // appAuth.checkPermission();
        ...
        vm.dateFilter = function () {
            ...
            return function (item) {
                ...
                var result = false;
                var DataDate = $filter('date')(new Date(item['TranDate']), "yyyy-MM-dd"); //new Date(item['TranDate']);
        ...
        //Get All Data List
        function GetCOAList() {
            accCOAResource.query({ 'id': param.id, 'FilterType': param.FilterType }).$promise.then(function (data) {
                vm.accCOAs = data;
