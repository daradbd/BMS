(function ()
{
    ...
    angular
        ...
        .controller("bankAccountModalCtrl", ["bankAccountOwnerTypeResource", "bankAccountTypeResource", "bankResource", "bankBranchResource", "bankAccountResource", "bankAccountFormData", "$uibModalInstance", "appAuth", bankAccountModalCtrl]);
    function bankAccountModalCtrl(bankAccountOwnerTypeResource, bankAccountTypeResource, bankResource, bankBranchResource, bankAccountResource,bankAccountFormData, $uibModalInstance, appAuth)
    {
        var vm = this;
        vm.bankAccounts = [];
        vm.banks = [];
            ...
            if (isValid)
            {
                bankAccountResource.save(vm.bankAccount).$promise.then(
                    function (data, responseHeaders)
                    {
                       // GetList();
                        vm.bankAccount = data;
                        $uibModalInstance.close(vm.bankAccount);
                        toastr.success("Save Successful");
                    }, function (error) {
                        // error handler
                        toastr.error("Data Save Failed!");
        ...
        vm.ListItemSelectClose = function (bankAccount) {
            $uibModalInstance.close(bankAccount);
        }
